...
def test_pharmacist(db_session):
    ...
    pharmacist = User(
        username="pharmacist@test.com", email="pharmacist@test.com", role="pharmacist"
    )
...
class TestValidationFlags:
    ...
    def test_low_confidence_ocr_flag(self, clinical_service, test_prescription):
        ...
        review = clinical_service.create_review(
            test_prescription, confidence_scores, extracted_data
        )
    ...
    def test_age_inappropriate_medication_pediatric(
        self, clinical_service, test_prescription
    ):
    ...
    def test_age_inappropriate_medication_geriatric(
        self, clinical_service, test_prescription
    ):
...
class TestPriorityDetermination:
    ...
    def test_critical_priority_for_critical_medication(
        self, clinical_service, test_prescription
    ):
        """Test that critical medications get CRITICAL priority"""
        confidence_scores = {"ocr_confidence": 0.95, "nlp_confidence": 0.95}
        extracted_data = {"medications": [{"name": "Insulin", "dosage": "10 units"}]}
    ...
    def test_high_priority_for_severe_issues(self, clinical_service, test_prescription):
        ...
        extracted_data = {"medications": [{"name": "Metformin", "dosage": "500mg"}]}
    ...
    def test_routine_priority_for_clean_prescription(
        self, clinical_service, test_prescription
    ):
...
class TestPharmacistReviewWorkflow:
    ...
    def test_submit_approved_review(
        self, clinical_service, test_prescription, test_pharmacist, db_session
    ):
    ...
    def test_submit_review_with_corrections(
        self, clinical_service, test_prescription, test_pharmacist
    ):
    ...
    def test_get_pending_reviews(
        ...
        for i in range(3):
            prescription = Prescription(
                prescription_id=f"RX-TEST-{i:03d}", patient_name=f"Patient {i}"
            )
    ...
    def test_get_pending_reviews_by_priority(self, clinical_service):
        ...
        prescription = Prescription(
            prescription_id="RX-CRITICAL-001", patient_name="Critical Patient"
        )
        db.session.add(prescription)
        db.session.commit()
        ...
        assert all(
            r.priority == ReviewPriority.CRITICAL.value for r in critical_reviews
        )
...
class TestClinicalMetrics:
    ...
    def test_get_review_metrics(
        ...
        for i in range(5):
            prescription = Prescription(
                prescription_id=f"RX-METRICS-{i:03d}", patient_name=f"Patient {i}"
            )
            db.session.add(prescription)
            db.session.commit()
