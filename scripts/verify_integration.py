...
class IntegrationVerifier:
    ...
    def __init__(self, verbose=False):
        ...
        self.results = {"passed": [], "failed": [], "warnings": [], "total": 0}
    ...
    def check_config_files(self) -> Tuple[bool, str]:
        ...
        files = ["src/config/database.py", "src/config/production.py"]
    ...
    def check_service_files(self) -> Tuple[bool, str]:
        ...
        files = ["src/services/auth_service.py", "src/services/monitoring_service.py"]
        missing = [f for f in files if not Path(f).exists()]
        if missing:
            return False, f"Missing files: {', '.join(missing)}"
    ...
    def check_route_files(self) -> Tuple[bool, str]:
        ...
        files = ["src/routes/auth_routes.py", "src/routes/health_routes.py"]
        missing = [f for f in files if not Path(f).exists()]
        if missing:
            return False, f"Missing files: {', '.join(missing)}"
    ...
    def check_util_files(self) -> Tuple[bool, str]:
        ...
        files = ["src/utils/__init__.py", "src/utils/error_handlers.py"]
        missing = [f for f in files if not Path(f).exists()]
        if missing:
            return False, f"Missing files: {', '.join(missing)}"
    ...
    def check_migration_files(self) -> Tuple[bool, str]:
        ...
        files = ["migrations/env.py", "alembic.ini"]
        missing = [f for f in files if not Path(f).exists()]
        if missing:
            return False, f"Missing files: {', '.join(missing)}"
    ...
    def check_metrics_collection(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            if "cpu_percent" in metrics and "memory_percent" in metrics:
                return (
                    True,
                    f"Metrics collection works (CPU: {metrics['cpu_percent']}%)",
                )
            else:
...
def main():
    ...
    verifier.run_check(
        "Monitoring Service Import", verifier.check_monitoring_service_import
    )
