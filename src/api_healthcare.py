...
from ehr_integration import EHRIntegrationService, EHRAuthenticator, EpicConnector, CernerConnector
        ...
        if ehr_system:
            try:
                sync_result = ehr_service.sync_prescription(
                    ehr_system=ehr_system,
                    prescription_data=fhir_bundle.dict()["entry"][2]["resource"]  # MedicationRequest
                )
            except Exception as e:
