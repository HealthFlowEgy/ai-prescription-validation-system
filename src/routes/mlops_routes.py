...
def get_monitoring_metrics():
    ...
    try:
        ...
        return (
            jsonify({"success": True, "metrics": metrics, "time_window_hours": hours}),
            200,
        )
    except Exception as e:
...
def check_drift():
    ...
    try:
        ...
        if drift_type == "data":
            ...
            drift_result = drift_service.check_prediction_drift(
                model_name, model_version
            )
        else:
            return (
                jsonify(
                    {"success": False, "error": f"Invalid drift_type: {drift_type}"}
                ),
                400,
            )
...
def stop_ab_test(test_id: int):
    ...
    try:
        ...
        return (
            jsonify(
                {"success": True, "message": f"A/B test stopped. Winner: {winner}"}
            ),
            200,
        )
    except ValueError as e:
