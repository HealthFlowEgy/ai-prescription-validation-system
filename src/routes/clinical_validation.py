...
def get_review_queue():
    ...
    try:
        ...
        reviews = clinical_service.get_pending_reviews(
            pharmacist_id=pharmacist_id, priority=priority, limit=limit
        )
        ...
        return (
            jsonify({"status": "error", "message": "Failed to fetch review queue"}),
            500,
        )
...
def get_review(review_id: str):
    ...
    try:
        ...
        return jsonify({"status": "error", "message": "Failed to fetch review"}), 500
...
def assign_review(review_id: str):
    ...
    try:
        ...
        updated_review = clinical_service.assign_review(
            review=review, pharmacist_id=g.current_user.id
        )
        ...
        return jsonify({"status": "error", "message": "Failed to assign review"}), 500
...
def submit_review(review_id: str):
    ...
    try:
        ...
        if review.assigned_to != g.current_user.id:
            return (
                jsonify({"status": "error", "message": "Review not assigned to you"}),
                403,
            )
...
def create_safety_alert():
    ...
    try:
        ...
        return (
            jsonify({"status": "error", "message": "Failed to create safety alert"}),
            500,
        )
...
def get_safety_alerts():
    ...
    try:
        ...
        # Execute query
        alerts = query.order_by(SafetyAlert.detected_at.desc()).limit(limit).all()
        ...
        return (
            jsonify({"status": "error", "message": "Failed to fetch safety alerts"}),
            500,
        )
...
def get_my_review_stats():
    ...
    try:
        ...
        my_reviews = PharmacistReview.query.filter_by(reviewed_by=pharmacist_id).all()
        ...
        completed_week = [
            ...
            for r in my_reviews
            if r.completed_at
            and r.completed_at >= datetime.utcnow() - timedelta(days=7)
        ]
        ...
        stats = {
            ...
            "pending_assigned_to_me": PharmacistReview.query.filter_by(
                assigned_to=pharmacist_id, status="PENDING"
            ).count(),
            "avg_review_time_minutes": (
                sum(
                    r.time_to_review_seconds
                    for r in my_reviews
            ...
            "avg_accuracy_score": (
                sum(r.accuracy_score for r in my_reviews if r.accuracy_score)
                / len([r for r in my_reviews if r.accuracy_score])
                if my_reviews
                else 0
            ),
            "approval_rate": (
                len([r for r in my_reviews if r.status == ReviewStatus.APPROVED.value])
                / len(my_reviews)
                if my_reviews
                else 0
            ),
        ...
        return (
            jsonify({"status": "error", "message": "Failed to fetch statistics"}),
            500,
        )
