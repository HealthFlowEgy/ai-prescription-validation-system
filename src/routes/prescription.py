...
def process_prescription(prescription_id):
    ...
    try:
        ...
        if prescription.processing_status != ProcessingStatus.UPLOADED:
            return (
                jsonify({"error": "Prescription already processed or processing"}),
                400,
            )
        ...
        if "error" in ocr_results:
            ...
            db.session.commit()
            return (
                jsonify({"error": f'OCR processing failed: {ocr_results["error"]}'}),
                500,
            )
        ...
        prescription.extracted_entities = json.dumps(
            nlp_results.get("extracted_entities", {})
        )
        ...
        if parsed_data.get("prescription_date"):
            try:
                ...
                logger.warning(
                    f"Could not parse prescription date: {parsed_data.get('prescription_date')}"
                )
        ...
        create_audit_log(
            ...
            new_values={
                ...
                "processing_time": nlp_results.get("processing_metadata", {}).get(
                    "processing_time", 0
                ),
            },
        )
            ...
            jsonify(
                {
                    ...
                    "results": {
                        ...
                        "medications_count": len(medications_info),
                        "ocr_confidence": ocr_results.get("average_confidence", 0),
                        "processing_time": nlp_results.get(
                            "processing_metadata", {}
...
def validate_prescription(prescription_id):
    ...
    try:
        ...
        if prescription.processing_status != ProcessingStatus.COMPLETED:
            return (
                jsonify({"error": "Prescription must be processed before validation"}),
                400,
            )
            ...
            jsonify(
                {
                    ...
                    "summary": {
                        "total_issues": validation_results["total_issues"],
                        "high_severity_issues": validation_results[
                            "high_severity_issues"
                        ],
                        "medium_severity_issues": validation_results[
                            "medium_severity_issues"
                        ],
                        "low_severity_issues": validation_results[
                            "low_severity_issues"
                        ],
                        "drug_interactions": validation_results["drug_interactions"],
                    },
...
@prescription_bp.route(
    "/prescriptions/<prescription_id>/validation-summary", methods=["GET"]
)
...
def get_prescription_medications(prescription_id):
    ...
    try:
        ...
        logger.error(
            f"Error getting medications for prescription {prescription_id}: {str(e)}"
        )
        return jsonify({"error": "Internal server error"}), 500
...
def get_prescription_audit_log(prescription_id):
    ...
    try:
        ...
        audit_logs = (
            AuditLog.query.filter_by(prescription_id=prescription.id)
        ...
        logger.error(
            f"Error getting audit log for prescription {prescription_id}: {str(e)}"
        )
        return jsonify({"error": "Internal server error"}), 500
