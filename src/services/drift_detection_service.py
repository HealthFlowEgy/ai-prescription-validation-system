...
class ABTest(db.Model):
    ...
    def __repr__(self):
        return (
            f"<ABTest {self.test_id}: {self.champion_model} vs {self.challenger_model}>"
        )
...
class DriftDetectionService:
    ...
    def detect_data_drift(
        ...
        if (
            len(baseline_preds) < self.MIN_SAMPLES
            or len(current_preds) < self.MIN_SAMPLES
        ):
            self.logger.warning(
                f"Insufficient samples for drift detection: {len(baseline_preds)}, {len(current_preds)}"
            )
            return None

        # Calculate PSI for confidence scores
        baseline_confidences = [
            p.confidence_score for p in baseline_preds if p.confidence_score
        ]
        current_confidences = [
            p.confidence_score for p in current_preds if p.confidence_score
        ]
    ...
    def detect_prediction_drift(
        self, model_name: str, model_version: str, lookback_days: int = 7
    ) -> Optional[DriftDetection]:
        """
        ...
        early_confidences = [
            p.confidence_score for p in early_preds if p.confidence_score
        ]
        recent_confidences = [
            p.confidence_score for p in recent_preds if p.confidence_score
        ]
...
class ABTestingService:
    ...
    def route_prediction(self, test_id: str, request_id: str) -> Tuple[str, str]:
        """
    ...
    def record_result(
        ...
        if model_name == test.champion_model and model_version == test.champion_version:
            ...
            and model_version == test.challenger_version
        ):
    ...
    def _check_test_completion(self, test: ABTest):
        ...
        if (
            test.champion_samples < test.min_sample_size
            or test.challenger_samples < test.min_sample_size
        ):
            return
        ...
        if test.metrics:
            for metric_name in test.metrics.get("champion", {}).keys():
                ...
                if (
                    len(champion_values) >= test.min_sample_size
                    and len(challenger_values) >= test.min_sample_size
                ):
                    # Perform t-test
    ...
    def get_test_results(self, test_id: str) -> Dict[str, Any]:
        ...
        results = {
            ...
            "duration_days": (
                (test.completed_at - test.started_at).days
                if test.completed_at
                else (datetime.utcnow() - test.started_at).days
            ),
            "champion": {
                    ...
                    if metric_name in test.metrics["challenger"]:
                        ...
                        if len(champion_vals) > 1 and len(challenger_vals) > 1:
                            ...
                            results["statistical_tests"][metric_name] = {
                                ...
                                "effect_size": (
                                    np.mean(challenger_vals) - np.mean(champion_vals)
                                )
                                / np.std(champion_vals + challenger_vals),
                            }
...
def pause_ab_test(test_id: str):
    ...
    return jsonify({"status": "success", "message": "Test paused"}), 200
...
def resume_ab_test(test_id: str):
    ...
    return jsonify({"status": "success", "message": "Test resumed"}), 200
...
def cancel_ab_test(test_id: str):
    ...
    return jsonify({"status": "success", "message": "Test cancelled"}), 200
...
def example_drift_and_ab_testing():
    ...
    if results["metrics"]:
        print(
            f"Champion accuracy: {results['metrics']['champion'].get('accuracy', {}).get('mean', 0):.3f}"
        )
        print(
            f"Challenger accuracy: {results['metrics']['challenger'].get('accuracy', {}).get('mean', 0):.3f}"
        )
