...
class SnowstormService:
    ...
    def search_drug_concept(self, drug_name: str) -> Optional[Dict]:
        ...
        try:
            ...
            if concepts:
                ...
                logger.info(
                    f"Found SNOMED concept for '{drug_name}': {best_match['conceptId']} - {best_match['fsn']['term']}"
                )
                return best_match
            else:
    ...
    def get_drug_interactions(self, drug_concept_id: str) -> List[Dict]:
        ...
        try:
            ...
            params = {"activeFilter": True, "limit": 100}
            ...
            logger.error(
                f"Error getting drug interactions for concept {drug_concept_id}: {str(e)}"
            )
            return []

    def check_drug_drug_interactions(
        self, drug_names: List[str]
    ) -> List[SnowstormDrugInteraction]:
        ...
        for i, drug1_name in enumerate(drug_list):
            for drug2_name in drug_list[i + 1 :]:
                ...
                interaction = self._check_pair_interaction(
                    drug1_name, drug1_concept, drug2_name, drug2_concept
                )
    ...
    def _check_pair_interaction(
        self, drug1_name: str, drug1_concept: Dict, drug2_name: str, drug2_concept: Dict
    ) -> Optional[SnowstormDrugInteraction]:
        ...
        try:
            ...
            logger.error(
                f"Error checking pair interaction between {drug1_name} and {drug2_name}: {str(e)}"
            )
            return None

    def _analyze_relationships_for_interactions(
        self,
        drug1_name: str,
        drug1_concept: Dict,
        drug1_rels: List[Dict],
        drug2_name: str,
        drug2_concept: Dict,
        drug2_rels: List[Dict],
    ) -> Optional[SnowstormDrugInteraction]:
        ...
        known_interactions = self._check_known_interactions(
            drug1_name, drug1_active_ingredients, drug2_name, drug2_active_ingredients
        )
    ...
    def _check_known_interactions(
        self,
        drug1_name: str,
        drug1_ingredients: List[str],
        drug2_name: str,
        drug2_ingredients: List[str],
    ) -> Optional[Dict]:
    ...
    def search_concepts_by_ecl(
        self, ecl_expression: str, limit: int = 50
    ) -> List[Dict]:
        """Search concepts using Expression Constraint Language (ECL)"""
        try:
            url = f"{self.base_url}/{self.branch}/concepts"
            params = {"ecl": ecl_expression, "activeFilter": True, "limit": limit}
            ...
            logger.error(
                f"Error searching concepts with ECL '{ecl_expression}': {str(e)}"
            )
            return []
