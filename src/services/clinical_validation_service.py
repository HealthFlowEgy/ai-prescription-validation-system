...
class ClinicalValidationService:
    ...
    def _determine_priority(
        self, flags: List[ClinicalFlag], extracted_data: Dict[str, Any]
    ) -> ReviewPriority:
        ...
        has_critical_med = any(
            flag.flag_type == ValidationFlag.CRITICAL_MEDICATION for flag in flags
        )
        has_severe = any(flag.severity == SafetySeverity.SEVERE for flag in flags)
        ...
        moderate_flags = [
            flag for flag in flags if flag.severity == SafetySeverity.MODERATE
        ]
    ...
    def _check_missing_information(self, extracted_data: Dict[str, Any]) -> List[str]:
    ...
    def _check_age_appropriateness(
        self, medications: List[Dict[str, Any]], patient_age: int
    ) -> List[ClinicalFlag]:
    ...
    def assign_review(
        self, review: PharmacistReview, pharmacist_id: int
    ) -> PharmacistReview:
    ...
    def submit_review(
        ...
        if corrected_data:
            ...
            review.num_corrections = self._count_corrections(
                review.original_data, corrected_data
            )
    ...
    def get_pending_reviews(
        ...
        query = PharmacistReview.query.filter_by(status=ReviewStatus.PENDING.value)
    ...
    def get_review_metrics(
        ...
        return {
            ...
            "pending_reviews": len(
                [r for r in reviews if r.status == ReviewStatus.PENDING.value]
            ),
            "approval_rate": (
                len(
                    [
            ...
            "avg_corrections": (
                sum(r.num_corrections for r in completed_reviews)
                / len(completed_reviews)
                if completed_reviews
                else 0
            ),
            "avg_accuracy": (
                sum(r.accuracy_score for r in completed_reviews if r.accuracy_score)
                / len([r for r in completed_reviews if r.accuracy_score])
                if completed_reviews
                else 0
            ),
            "critical_reviews": len(
                [r for r in reviews if r.priority == ReviewPriority.CRITICAL.value]
            ),
