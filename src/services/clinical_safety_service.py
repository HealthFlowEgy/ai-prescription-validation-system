...
class ClinicalValidationService:
    ...
    CRITICAL_MEDICATIONS = [
        ...
        "amiodarone",  # Cardiac
        "chemotherapy",
        "methotrexate",  # Cancer/Immunosuppressants
        "opioids",
        "morphine",
        "fentanyl",  # Opioids
        "lithium",
        "clozapine",  # Psychiatric
    ]
    ...
    def validate_prescription(
        self, ocr_result: Dict, nlp_result: Dict, patient_context: Optional[Dict] = None
    ) -> Dict:
        """
    ...
    def _validate_critical_medications(self, nlp_result: Dict) -> List[ValidationFlag]:
        ...
        for med in medications:
            ...
            is_critical = any(
                crit_med in med_name for crit_med in self.CRITICAL_MEDICATIONS
            )
    ...
    def _validate_dosages(self, nlp_result: Dict) -> List[ValidationFlag]:
        ...
        for med in medications:
            ...
            if not dosage:
                flags.append(
                    ValidationFlag(
                        ...
                        details={"medication": med_name, "missing_field": "dosage"},
                        requires_pharmacist_review=True,
                        timestamp=datetime.utcnow(),
                    )
                )
            ...
            if med_name in self.dosage_database:
                ...
                if dosage_value:
                    if (
                        dosage_value < normal_range.min_daily_dose
                        or dosage_value > normal_range.max_daily_dose
                    ):
    ...
    def _validate_required_fields(self, nlp_result: Dict) -> List[ValidationFlag]:
        ...
        required_fields = ["patient_name", "medications", "prescriber_name", "date"]
    ...
    def _check_drug_interactions(
        self, nlp_result: Dict, patient_context: Dict
    ) -> List[ValidationFlag]:
    ...
    def _determine_validation_status(self, flags: List[ValidationFlag]) -> str:
        """Determine overall validation status"""
        if not flags:
            return "approved"

        # Check for critical flags
        has_critical = any(f.severity == ValidationSeverity.CRITICAL for f in flags)
        if has_critical:
            return "rejected"
    ...
    def _generate_summary(self, flags: List[ValidationFlag]) -> str:
        ...
        critical_count = sum(
            1 for f in flags if f.severity == ValidationSeverity.CRITICAL
        )
...
if __name__ == "__main__":
    ...
    ocr_result = {
        "confidence": 0.82,
        "text": "...",
        "field_confidences": {"medication": 0.91, "dosage": 0.75, "patient_name": 0.88},
    }

    nlp_result = {
        "medications": [
            {"name": "Warfarin", "dosage": "5mg daily", "confidence": 0.93}
        ],
    ...
    patient_context = {"current_medications": ["aspirin"], "allergies": [], "age": 65}

    result = validator.validate_prescription(ocr_result, nlp_result, patient_context)
