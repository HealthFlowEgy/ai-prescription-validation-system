...
class PharmacyDirectory:
    ...
    def search_pharmacies(
        ...
        for pharmacy in self.pharmacies.values():
            ...
            if filters:
                if (
                    filters.get("network")
                    and pharmacy.network not in filters["network"]
                ):
                    continue

                if (
                    filters.get("accepts_eprescribe")
                    and not pharmacy.accepts_eprescribe
                ):
                    continue

                if (
                    filters.get("accepts_controlled")
                    and not pharmacy.accepts_controlled
                ):
                    continue

                if filters.get("24_hour") and "24 hours" not in pharmacy.hours.get(
                    "Monday", ""
                ):
                    continue
    ...
    def get_pharmacy_availability(self, ncpdp_id: str) -> Dict:
        ...
        if pharmacy.status != PharmacyStatus.ACTIVE:
            return {
                "available": False,
                "reason": f"Pharmacy status: {pharmacy.status.value}",
            }
    ...
    def route_prescription(
        ...
        if preferences.preferred_pharmacy_id:
            ...
            if preferred and preferred.status == PharmacyStatus.ACTIVE:
                distance = geodesic(
                    patient_location, (preferred.latitude, preferred.longitude)
                ).miles
                ...
                if distance <= preferences.max_distance_miles:
                    ...
                    return [
                        self._create_route_option(
                            preferred, distance, rank=1, reason="Preferred pharmacy"
                        )
                    ]
        ...
        filters = {
            "accepts_eprescribe": True,
            "network": (
                preferences.preferred_networks
                if preferences.preferred_networks
                else None
            ),
...
class SurescriptsConnector:
    ...
    def send_prescription(self, ncpdp_script_xml: str, pharmacy_ncpdp_id: str) -> Dict:
        """
...
class PrescriptionTransmissionService:
    ...
    def __init__(self, router: PharmacyRouter, connector: SurescriptsConnector):
        self.router = router
    ...
    def transmit_prescription(
        ...
        routing_options = self.router.route_prescription(
            patient_location=patient_location, preferences=preferences
        )
        ...
        for attempt in range(max_retries):
            for option in routing_options:
                ...
                result = self.connector.send_prescription(
                    ncpdp_script_xml=prescription_xml, pharmacy_ncpdp_id=pharmacy_id
                )
...
if __name__ == "__main__":
    ...
    options = router.route_prescription(
        patient_location=boston_location, preferences=preferences
    )
