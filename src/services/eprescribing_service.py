...
class NCPDPScriptBuilder:
    ...
    def build_status(
        ...
        root = ET.Element(
            "Message", attrib={"xmlns": self.NAMESPACE, "version": self.VERSION}
        )
        ...
        logger.info(
            f"Built STATUS message {self.message_id} for {reference_message_id}"
        )
...
class NCPDPScriptParser:
    ...
    def parse_message(self, xml_string: str) -> Dict:
        ...
        try:
            ...
            logger.info(
                f"Parsed {message['message_type']} message {header['message_id']}"
            )
    ...
    def _parse_header(self, root: ET.Element, namespace: Dict) -> Dict:
        ...
        return {
            ...
            "relates_to": self._get_text(
                header_elem, "ns:RelatesToMessageID", namespace
            ),
        }
    ...
    def _parse_body(self, body_elem: ET.Element, namespace: Dict) -> Dict:
        ...
        for child in body_elem:
            ...
            if tag_name == "NewRx":
                return {"type": "NEWRX", "data": self._parse_newrx(child, namespace)}
                ...
                return {"type": "STATUS", "data": self._parse_status(child, namespace)}
            elif tag_name == "Error":
                return {"type": "ERROR", "data": self._parse_error(child, namespace)}
    ...
    def _parse_rxchange(self, rxchange_elem: ET.Element, namespace: Dict) -> Dict:
        ...
        return {
            "change_reason_code": self._get_text(
                change_request, "ns:ChangeReasonCode", namespace
            ),
            "change_reason_text": self._get_text(
                change_request, "ns:ChangeReasonText", namespace
            ),
            "prescriber": self._parse_prescriber(
...
if __name__ == "__main__":
    ...
    print(
        f"Patient: {parsed['body']['data']['patient']['first_name']} {parsed['body']['data']['patient']['last_name']}"
    )
