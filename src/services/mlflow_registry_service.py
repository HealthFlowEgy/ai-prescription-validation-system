...
class MLflowModelService:
    ...
    def __init__(self, tracking_uri: Optional[str] = None):
        ...
        self.logger.info(
            f"Initialized MLflow service with tracking URI: {self.tracking_uri}"
        )
    ...
    def promote_model(
        ...
        if stage == ModelStage.PRODUCTION and archive_existing:
            ...
            for mv in existing_prod_versions:
                self.client.transition_model_version_stage(
                    name=model_name, version=mv.version, stage=ModelStage.ARCHIVED.value
                )
                self.logger.info(
        ...
        self.logger.info(f"Promoted {model_name} version {version} to {stage.value}")
    ...
    def compare_models(
        ...
        # Compare metrics
        metrics_comparison = {}
        all_metrics = set(metadata1["metrics"].keys()) | set(
            metadata2["metrics"].keys()
        )
        ...
        return {
            ...
            "parameters_diff": {
                ...
                for k in set(metadata1["params"].keys())
                | set(metadata2["params"].keys())
    ...
    def add_model_alias(self, model_name: str, version: str, alias: str) -> None:
        """
...
class OCRModelService:
    ...
    def get_production_model(self):
        ...
        return self.mlflow_service.load_model(
            self.model_name, stage=ModelStage.PRODUCTION
        )
