...
class AuthService:
    ...
    @staticmethod
    def generate_token(
        user_id: int, role: str, expires_in: int = None
    ) -> Tuple[str, datetime]:
        """
        ...
        token = jwt.encode(payload, AuthService.get_secret_key(), algorithm="HS256")

        return token, expiration

    @staticmethod
    def generate_refresh_token(
        user_id: int, role: str = "user"
    ) -> Tuple[str, datetime]:
        """
    ...
    def decode_token(token: str) -> Optional[Dict]:
        ...
        try:
            payload = jwt.decode(
                token, AuthService.get_secret_key(), algorithms=["HS256"]
            )
            return payload
    ...
    def authenticate_user(email: str, password: str) -> Optional[User]:
        ...
        if not user:
            current_app.logger.warning(
                f"Authentication failed: User not found for {email}"
            )
            return None

        if not user.is_active:
            current_app.logger.warning(
                f"Authentication failed: User {email} is inactive"
            )
            return None

        if not user.password_hash:
            current_app.logger.warning(
                f"Authentication failed: No password set for {email}"
            )
            return None
