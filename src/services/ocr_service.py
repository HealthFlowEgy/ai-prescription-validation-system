...
class OCRService:
    ...
    def extract_text_from_image(
        self, image: np.ndarray, preprocessing_method: str = "adaptive"
    ) -> Dict:
        """
        ...
        try:
            ...
            text = pytesseract.image_to_string(
                deskewed, lang="eng", config=custom_config
            )
            ...
            logger.info(
                f"OCR completed in {processing_time:.3f}s with {avg_confidence:.1f}% confidence"
            )
            return result
    ...
    def process_pdf_file(self, pdf_path: str) -> Dict:
        ...
        try:
            ...
            results["average_confidence"] = (
                total_confidence / len(results["pages"]) if results["pages"] else 0
            )
            results["total_words"] = sum(
                page["word_count"] for page in results["pages"]
            )

            logger.info(
                f"PDF processing completed: {len(pages)} pages in {results['processing_time']:.3f}s"
            )
            return results
