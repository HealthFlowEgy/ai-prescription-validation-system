...
class AuditLoggingService:
    ...
    def log_access(
        ...
        logger.info(
            f"Audit log created: {action} on {resource_type} {resource_id} by {username}"
        )
    ...
    def get_failed_access_attempts(hours: int = 24) -> List[AuditLog]:
        ...
        return (
            AuditLog.query.filter(
                AuditLog.result.in_(["FAILURE", "DENIED"]), AuditLog.timestamp >= since
            )
            .order_by(AuditLog.timestamp.desc())
            .all()
        )
    ...
    def get_emergency_access_logs(days: int = 7) -> List[AuditLog]:
        ...
        return (
            AuditLog.query.filter(
                AuditLog.emergency_access == True, AuditLog.timestamp >= since
            )
            .order_by(AuditLog.timestamp.desc())
            .all()
        )
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            try:
                ...
                resource_id = (
                    kwargs.get("id")
                    or kwargs.get("prescription_id")
                    or kwargs.get("patient_id")
                    or getattr(response, "id", "unknown")
                )
