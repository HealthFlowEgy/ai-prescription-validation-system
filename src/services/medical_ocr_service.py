...
class MedicalOCRService:
    ...
    def extract_text_from_image(self, image_path: str) -> str:
        ...
        try:
            ...
            encoding = self._processor(images=image, return_tensors="pt").to(
                self.device
            )
    ...
    def process_prescription(
        self, image_path: str, validate_type: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if validate_type:
                predicted_label, confidence = self.classify_prescription_zero_shot(
                    extracted_text
                )
            ...
            result["success"] = True
            logger.info(
                f"Processing successful. Confidence: {result['confidence']:.2%}"
            )
    ...
    def extract_structured_data(self, text: str) -> Dict[str, Any]:
        ...
        for line in lines:
            ...
            # Extract doctor name
                ...
                match = re.search(
                    r"(?:doctor|dr\.)[:\s]+([A-Za-z\s]+)", line, re.IGNORECASE
                )
                if match:
    ...
    def _parse_medication_line(self, line: str) -> Optional[Dict[str, str]]:
        ...
        med = {"drug_name": "", "dosage": "", "frequency": "", "duration": ""}
        ...
        duration_match = re.search(
            r"(\d+\s+(?:days?|weeks?|months?))", line, re.IGNORECASE
        )
        if duration_match:
