...
class MetricsCollector:
    ...
    def record_request(
        ...
        if self.counters["total_requests"] > 0:
            error_rate = (
                self.counters["failed_requests"] / self.counters["total_requests"]
            )
    ...
    def _calculate_current_error_rate(self) -> float:
        ...
        return self.counters["failed_requests"] / self.counters["total_requests"]
    ...
    def _calculate_throughput(self) -> float:
        ...
        recent_requests = [
            m for m in self.metrics["response_times"] if m.timestamp > one_minute_ago
        ]
...
class DriftDetector:
    ...
    def check_drift(self, current_metrics: Dict) -> Dict:
        ...
        if "accuracy" in current_metrics and "accuracy" in self.baseline_metrics:
            accuracy_drift = (
                self.baseline_metrics["accuracy"] - current_metrics["accuracy"]["mean"]
            )
...
class AlertManager:
    ...
    def evaluate_alerts(
        self, metrics: Dict, drift_result: Optional[Dict] = None
    ) -> List[Alert]:
        """
    ...
    def _check_threshold(
        self, rule: Dict, metrics: Dict, drift_result: Optional[Dict]
    ) -> bool:
    ...
    def _create_alert(self, rule_name: str, rule: Dict, metrics: Dict) -> Alert:
        ...
        return Alert(
            ...
            description=self._generate_alert_description(
                rule_name, current_value, rule["threshold"]
            ),
            ...
            threshold_value=(
                rule["threshold"] if isinstance(rule["threshold"], (int, float)) else 0
            ),
            timestamp=datetime.utcnow(),
        )
    ...
    def _is_in_cooldown(self, rule_name: str) -> bool:
        ...
        recent_alerts = [a for a in self.alert_history if rule_name in a.alert_id]
    ...
    def _generate_alert_description(
        self, rule_name: str, current_value: float, threshold: float
    ) -> str:
        ...
        return descriptions.get(rule_name, f"Metric {rule_name} threshold exceeded")
...
class MonitoringService:
    ...
    def record_prediction(
        ...
        self.metrics_collector.record_request(
            response_time_ms, confidence_score, success, metadata
        )
    ...
    def check_system_health(self) -> Dict:
        ...
        # Evaluate alerts
        new_alerts = self.alert_manager.evaluate_alerts(current_metrics, drift_result)

        # Determine overall health status
        health_status = self._determine_health_status(
            current_metrics, drift_result, new_alerts
        )
    ...
    def _determine_health_status(
        self, metrics: Dict, drift_result: Dict, new_alerts: List[Alert]
    ) -> str:
        ...
        high_alerts = [a for a in new_alerts if a.severity == "high"]
...
if __name__ == "__main__":
    ...
    baseline = {"accuracy": 0.94, "confidence": 0.90, "response_time": 500}
