...
class ModelRegistry:
    ...
    def register_model(
        ...
        with mlflow.start_run(experiment_id=self.experiment_id) as run:
            ...
            # Get version
            model_version = self.client.search_model_versions(f"name='{model_name}'")[
                0
            ].version
    ...
    def transition_model_stage(
        ...
        if archive_existing and stage == "Production":
            ...
            for model in existing_prod_models:
                ...
                logger.info(f"Archived {model_name} version {model.version}")
    ...
    def compare_models(self, model_name: str, versions: List[str]) -> Dict:
        """
...
class ModelPerformanceTracker:
    ...
    def log_prediction(
        ...
        if ground_truth:
            metric_entry["correct"] = prediction.get("result") == ground_truth.get(
                "result"
            )
    ...
    def _flush_metrics(self):
        ...
        # Aggregate metrics
        avg_confidence = sum(m["confidence"] for m in self.metrics_buffer) / len(
            self.metrics_buffer
        )

        avg_latency = sum(m["prediction_time_ms"] for m in self.metrics_buffer) / len(
            self.metrics_buffer
        )

        # Calculate accuracy if ground truth available
        accuracy_metrics = [
            m for m in self.metrics_buffer if m["ground_truth_available"]
        ]

        if accuracy_metrics:
            accuracy = sum(m["correct"] for m in accuracy_metrics) / len(
                accuracy_metrics
            )
        else:
            accuracy = None
    ...
    def detect_drift(
        ...
        if drift_detected:
            logger.warning(f"Drift detected for {model_name}: {drift_details}")
