...
class ModelMetric(db.Model):
    ...
    def __repr__(self):
        return (
            f"<ModelMetric {self.model_name} {self.metric_type}: {self.metric_value}>"
        )
...
class ModelMonitoringService:
    ...
    def _check_realtime_alerts(self, prediction: ModelPrediction):
        ...
        if (
            ...
            and prediction.confidence_score < self.CONFIDENCE_THRESHOLD_LOW
        ):
            self.create_alert(
    ...
    def calculate_aggregated_metrics(
        self, model_name: str, window_size_minutes: int = 60, lookback_hours: int = 24
    ) -> List[ModelMetric]:
        """
    ...
    def _group_into_windows(
        self, predictions: List[ModelPrediction], window_size_minutes: int
        ...
        for pred in predictions:
            ...
            window_start = pred.timestamp.replace(
                minute=(pred.timestamp.minute // window_size_minutes)
                * window_size_minutes,
                second=0,
                microsecond=0,
            )
    ...
    def _calculate_window_metrics(
        ...
        model_version = max(
            set(p.model_version for p in predictions),
            key=lambda v: sum(1 for p in predictions if p.model_version == v),
        )
        ...
        predictions_with_truth = [
            p for p in predictions if p.ground_truth and p.is_correct
        ]
            ...
            if correct_counts:
                metrics.append(
                    ModelMetric(
                        ...
                        std_dev=(
                            statistics.stdev(correct_counts)
    ...
    def detect_performance_degradation(
        ...
        if degraded:
            return {
                ...
                "change": change,
                "percent_change": (
                    (change / historical_avg * 100) if historical_avg != 0 else 0
                ),
                "degraded": True,
            }
    ...
    def create_alert(
        ...
        if severity == AlertSeverity.CRITICAL:
            self.logger.critical(
                f"Model Alert: {message}", extra={"alert_id": alert.alert_id}
            )
    ...
    def get_model_dashboard_data(
        ...
        metrics = ModelMetric.query.filter(
            ModelMetric.model_name == model_name, ModelMetric.window_start >= start_time
        ).all()

        # Get active alerts
        active_alerts = ModelAlert.query.filter(
            ModelAlert.model_name == model_name, ModelAlert.status == "OPEN"
        ).all()
...
if __name__ == "__main__":
    ...
    metrics = monitoring.calculate_aggregated_metrics(
        model_name="medical-prescription-ocr", window_size_minutes=60, lookback_hours=24
    )
