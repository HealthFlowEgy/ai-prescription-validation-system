...
class DrugInteractionChecker:
    ...
    def __init__(self, snowstorm_url: str = "https://snowstorm.app.evidium.com"):
        ...
        logger.info(
            f"Initialized DrugInteractionChecker with Snowstorm server: {snowstorm_url}"
        )
    ...
    def check_interactions(
        ...
        try:
            ...
            snowstorm_interactions = (
                self.snowstorm_service.check_drug_drug_interactions(drug_names)
            )
            ...
            for snow_interaction in snowstorm_interactions:
                ...
                logger.info(
                    f"Found drug interaction via Snowstorm: {interaction.drug1} + {interaction.drug2} ({interaction.severity})"
                )
    ...
    def _fallback_interaction_check(
        self, medications: List[Medication]
    ) -> List[DrugInteraction]:
        ...
        if interactions:
            logger.warning(
                f"Using fallback interaction checking, found {len(interactions)} interactions"
            )
        ...
        if dosage_amount < dosage_range["min"]:
            issues.append(
                ValidationIssue(
                    ...
                    affected_fields=["dosage"],
                    recommendations=[
                        f'Consider increasing to at least {dosage_range["min"]}{dosage_range["unit"]}'
                    ],
                    confidence=0.8,
                )
            )
            ...
            issues.append(
                ValidationIssue(
                    ...
                    affected_fields=["dosage"],
                    recommendations=[
                        f'Consider reducing to maximum {dosage_range["max"]}{dosage_range["unit"]}'
                    ],
                    confidence=0.9,
                )
            )
...
class CompletenessValidator:
    ...
    def validate_prescription_completeness(
            ...
            if not value:
                issues.append(
                    ValidationIssue(
                        ...
                        affected_fields=[field],
                        recommendations=[
                            f'Consider providing {field.replace("_", " ")}'
                        ],
                        confidence=0.7,
                    )
                )
    ...
    def validate_medication_completeness(
            ...
            if not value:
                issues.append(
                    ValidationIssue(
                        ...
                        affected_fields=[field],
                        recommendations=[
                            f'Provide {field.replace("_", " ")} for {medication.drug_name}'
                        ],
                        confidence=1.0,
                    )
                )
            ...
            if not value:
                issues.append(
                    ValidationIssue(
                        ...
                        affected_fields=[field],
                        recommendations=[
                            f'Consider providing {field.replace("_", " ")} for {medication.drug_name}'
                        ],
                        confidence=0.7,
                    )
                )
...
class ValidationService:
    ...
    def validate_prescription(self, prescription_id: int) -> Dict:
        ...
        try:
            ...
            completeness_issues = (
                self.completeness_validator.validate_prescription_completeness(
                    prescription
                )
            )
            ...
            for medication in medications:
                ...
                med_completeness_issues = (
                    self.completeness_validator.validate_medication_completeness(
                        medication
                    )
                )
            ...
            response = {
                ...
                "drug_interactions": len(
                    [i for i in all_issues if i.issue_type == "drug_interaction"]
                ),
                ...
                "validation_results": [
                    {
                        ...
                        "recommendations": (
                            json.loads(result.recommendations)
            ...
            logger.info(
                f"Prescription validation completed: {prescription.prescription_id} - {overall_status.value}"
            )
            return response
    ...
    def get_validation_summary(self, prescription_id: int) -> Dict:
        ...
        try:
            ...
            validation_results = ValidationResult.query.filter_by(
                prescription_id=prescription_id
            ).all()
            ...
            summary = {
                ...
                "issues_by_severity": {
                    "high": len(
                        [r for r in validation_results if r.severity == "high"]
                    ),
                    "medium": len(
                        [r for r in validation_results if r.severity == "medium"]
                    ),
            ...
            logger.error(
                f"Error getting validation summary for prescription {prescription_id}: {str(e)}"
            )
