...
class TrialPrescription(db.Model):
    ...
    # Additional metadata
    prescription_complexity = Column(
        String(20), nullable=True
    )  # SIMPLE, MODERATE, COMPLEX
    handwriting_quality = Column(
        String(20), nullable=True
...
class ClinicalTrialService:
    ...
    def enroll_participant(
        ...
        self.logger.info(
            f"Enrolled participant: {participant.participant_id} in trial {trial.trial_id}"
        )
        return participant
    ...
    def record_ai_prediction(
        ...
        accuracy, field_accuracies, errors = self._calculate_accuracy(
            trial_prescription.gold_standard_data, ai_prediction
        )
    ...
    def _calculate_accuracy(
        self, gold_standard: Dict[str, Any], prediction: Dict[str, Any]
    ...
    def get_trial_statistics(self, trial: ClinicalTrial) -> Dict[str, Any]:
        ...
        trial_prescriptions = TrialPrescription.query.filter_by(trial_id=trial.id).all()

        if not trial_prescriptions:
            return {"total_prescriptions": 0, "progress_percentage": 0}

        # Filter prescriptions with AI predictions
        prescriptions_with_predictions = [
            tp for tp in trial_prescriptions if tp.ai_prediction is not None
        ]
        ...
        avg_field_accuracies = {
            field: statistics.mean(accs) for field, accs in field_accuracies.items()
        }
        ...
        for tp in trial_prescriptions:
            ...
            if tp.overall_accuracy is not None:
                complexity_breakdown[complexity]["accuracies"].append(
                    tp.overall_accuracy
                )
        ...
        return {
            ...
            "progress": {
                ...
                "progress_percentage": (
                    (
                        len(prescriptions_with_predictions)
                        / trial.target_sample_size
                        * 100
                    )
                    if trial.target_sample_size > 0
                    else 0
                ),
            },
            "accuracy_metrics": {
            ...
            "error_analysis": {
                ...
                "error_rate": (
                    len(all_errors) / len(prescriptions_with_predictions)
                    if prescriptions_with_predictions
                    else 0
                ),
            ...
            "confidence_analysis": {
                ...
                "confidence_range": (
                    (min(confidence_scores), max(confidence_scores))
                    ...
                    "trials": [
                        {
                            ...
                            "current_enrollment": TrialPrescription.query.filter_by(
                                trial_id=t.id
                            ).count(),
                        }
                        for t in trials
...
def example_clinical_trial_workflow():
    ...
    trial_prescription = trial_service.record_ai_prediction(
        trial_prescription, ai_prediction, confidence_scores
    )

    print(
        f"Recorded AI prediction with accuracy: {trial_prescription.overall_accuracy:.2%}"
    )
