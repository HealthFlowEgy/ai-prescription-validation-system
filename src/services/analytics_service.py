...
class PrescriptionAnalytics:
    ...
    def record_prescription(self, prescription_data: Dict):
        ...
        self.prescription_data.append(
            {**prescription_data, "recorded_at": datetime.utcnow()}
        )
    ...
    def calculate_volume_metrics(
        ...
        period_prescriptions = [
            p
            for p in self.prescription_data
        ...
        # Top providers
        top_providers = sorted(by_provider.items(), key=lambda x: x[1], reverse=True)[
            :10
        ]
        ...
        return {
            ...
            "by_provider": {
                ...
                "top_providers": [
                    {"provider_id": pid, "count": count} for pid, count in top_providers
                ],
            },
    ...
    def calculate_accuracy_metrics(
        ...
        period_prescriptions = [
            p
            for p in self.prescription_data
            if start_date <= p["recorded_at"] <= end_date and "ocr_confidence" in p
        ]
        ...
        # NLP accuracy
        nlp_confidences = [p.get("nlp_confidence", 0) for p in period_prescriptions]
        avg_nlp_confidence = statistics.mean(nlp_confidences) if nlp_confidences else 0

        # Low confidence rate
        low_confidence_count = sum(1 for c in ocr_confidences if c < 0.85)
        low_confidence_rate = low_confidence_count / len(period_prescriptions)

        # Error rate
        error_count = sum(1 for p in period_prescriptions if p.get("status") == "error")
        error_rate = error_count / len(period_prescriptions)

        # Manual review rate
        manual_review_count = sum(
            1 for p in period_prescriptions if p.get("requires_manual_review", False)
        )
        ...
        return {
            ...
            "ocr": {
                ...
                "std_dev": (
                    round(statistics.stdev(ocr_confidences), 3)
                    if len(ocr_confidences) > 1
                    else 0
                ),
            },
            "nlp": {"average_confidence": round(avg_nlp_confidence, 3)},
            "quality_metrics": {
    ...
    def calculate_performance_metrics(
        ...
        period_prescriptions = [
            p
            for p in self.prescription_data
            if start_date <= p["recorded_at"] <= end_date and "processing_time_ms" in p
        ]
        ...
        # Processing times
        processing_times = [p["processing_time_ms"] for p in period_prescriptions]
        ...
        return {
            ...
            "throughput": {
                "prescriptions_per_hour": round(
                    n / ((end_date - start_date).total_seconds() / 3600), 1
                )
            },
        }

    def analyze_error_patterns(self, start_date: datetime, end_date: datetime) -> Dict:
        """
...
class ClinicalAnalytics:
    ...
    def record_clinical_event(self, event_data: Dict):
        ...
        self.clinical_data.append({**event_data, "recorded_at": datetime.utcnow()})
    ...
    def analyze_drug_interactions(
        ...
        top_interactions = sorted(
            interaction_pairs.items(), key=lambda x: x[1], reverse=True
        )[:10]
        ...
        return {
            ...
            "top_interactions": [
                {"drugs": f"{pair[0]} + {pair[1]}", "count": count}
    ...
    def analyze_prescribing_patterns(
        ...
        top_medications = sorted(medications.items(), key=lambda x: x[1], reverse=True)[
            :10
        ]

        # Controlled substances
        controlled_count = sum(
            1 for e in provider_events if e.get("is_controlled", False)
        )
        ...
        return {
            ...
            "controlled_substances": {
                ...
                "percentage": (
                    round(controlled_count / len(provider_events) * 100, 1)
                    if provider_events
                    else 0
                ),
            },
            "top_medications": [
                {"medication": med, "count": count} for med, count in top_medications
            ],
        }
...
class PredictiveAnalytics:
    ...
    def identify_high_risk_prescriptions(self, prescriptions: List[Dict]) -> List[Dict]:
        """
    ...
    def generate_executive_summary(
        ...
        return {
            ...
            "key_metrics": {
                ...
                "automated_rate": accuracy.get("quality_metrics", {}).get(
                    "automated_rate", 0
                ),
                "error_rate": errors["error_rate"],
                "avg_processing_time_ms": performance.get("processing_time_ms", {}).get(
                    "mean", 0
                ),
            },
            "trends": {
            ...
            "quality": {
                "manual_review_rate": accuracy.get("quality_metrics", {}).get(
                    "manual_review_rate", 0
                ),
                "low_confidence_rate": accuracy.get("quality_metrics", {}).get(
                    "low_confidence_rate", 0
                ),
            },
            "performance": {
                "p95_latency_ms": performance.get("processing_time_ms", {}).get(
                    "p95", 0
                ),
                "throughput_per_hour": performance.get("throughput", {}).get(
                    "prescriptions_per_hour", 0
                ),
            },
...
if __name__ == "__main__":
    ...
    volume_metrics = prescription_analytics.calculate_volume_metrics(
        start_date, end_date
    )
    ...
    accuracy_metrics = prescription_analytics.calculate_accuracy_metrics(
        start_date, end_date
    )
    print("\nAccuracy Metrics:")
