...
class HL7MessageBuilder:
    ...
    def build_rde_o11_message(self, prescription_data: Dict) -> str:
        """
        ...
        for medication in prescription_data.get("medications", []):
            ...
            rxe.rxe_2 = (
                f"{medication.get('rxnorm_code', '')}^{medication.get('name', '')}^RXN"
            )
        ...
        logger.info(
            f"Built RDE^O11 message for prescription {prescription_data.get('prescription_id')}"
        )
...
class HL7Parser:
    ...
    def parse_message(self, hl7_string: str) -> HL7Message:
        ...
        try:
            # Parse using hl7apy
            msg = parse_message(
                hl7_string, validation_level=consts.VALIDATION_LEVEL.STRICT
            )
    ...
    def extract_prescription_data(self, hl7_message: HL7Message) -> Dict:
        ...
        if orc_segment:
            prescription_data["practitioner"] = self._extract_practitioner_from_orc(
                orc_segment
            )
    ...
    def _segment_to_dict(self, segment: Segment) -> Dict:
        ...
        segment_dict = {"segment_id": segment.name, "fields": {}}
...
class HL7MessageQueue:
    ...
    def get_queue_status(self) -> Dict:
        ...
        return {
            ...
            "success_count": len(
                [m for m in self.processed if m["status"] == "success"]
            ),
