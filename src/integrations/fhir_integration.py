...
class FHIRResourceBuilder:
    ...
    def build_patient_resource(
        ...
        # Build name
        name = HumanName(family=last_name, given=[first_name], use="official")
    ...
    def build_practitioner_resource(
        ...
        # Build identifiers
        identifiers = [
            Identifier(system=self.SYSTEM_NPI, value=npi, use="official"),
            Identifier(
                system=f"http://healthflow.ai/practitioner-id", value=practitioner_id
            ),
        ]

        # Build name
        name = HumanName(
            family=last_name, given=[first_name], prefix=["Dr."], use="official"
        )

        # Build contact
        telecom = []
        if phone:
            telecom.append(ContactPoint(system="phone", value=phone, use="work"))
    ...
    def build_bundle(
        ...
        for resource in resources:
            entry = BundleEntry(
                ...
                request=(
                    {"method": "POST", "url": resource.resource_type}
        ...
        bundle = Bundle(
            type=bundle_type, entry=entries, timestamp=datetime.utcnow().isoformat()
        )
...
class FHIRConverter:
    ...
    def fhir_to_prescription(self, bundle: Bundle) -> Dict:
        ...
        """
        prescription_data = {"patient": {}, "practitioner": {}, "medications": []}
        ...
        for entry in bundle.entry:
            ...
            if resource_type == "Patient":
                ...
                prescription_data["practitioner"] = self._extract_practitioner_data(
                    resource
                )
...
if __name__ == "__main__":
    ...
    print(
        f"FHIR Validation: {'✓ Valid' if validation_result['valid'] else '✗ Invalid'}"
    )
