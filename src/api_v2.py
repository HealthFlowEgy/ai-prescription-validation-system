...
try:
    ocr_model, ocr_version = model_registry.load_production_model(
        "prescription-ocr-v1"
    )
    nlp_model, nlp_version = model_registry.load_production_model(
        "prescription-nlp-v1"
    )
    logger.info(
        f"Loaded models - OCR: v{ocr_version.version}, "
        f"NLP: v{nlp_version.version}"
    )
...
def get_model_versions(model_name):
    ...
    try:
        versions = model_registry.client.search_model_versions(
            f"name='{model_name}'"
        )
    ...
    try:
        ...
        response_data = {
            ...
            "confidence": {
                ...
                "nlp": nlp_result["entities"][0]["confidence"] if nlp_result["entities"] else 0,
                "overall": (ocr_result["confidence"] + nlp_result["entities"][0]["confidence"]) / 2
            },
        ...
        logger.info(f"Processed prescription {prescription_id}: {safe_response['status']}")
...
if __name__ == '__main__':
    ...
    logger.info(f"Models loaded: OCR={ocr_model is not None}, NLP={nlp_model is not None}")
