...
class EncryptionService:
    ...
    def __init__(self, master_key: Optional[str] = None):
        ...
        self.master_key = master_key or os.environ.get(
            "PHI_ENCRYPTION_KEY", self._generate_key()
        )
    ...
    def anonymize_dict(cls, data: dict) -> dict:
        ...
        for key, value in data.items():
            if isinstance(value, str):
                ...
                anonymized[key] = [
                    (
                        cls.anonymize_dict(item)
                        if isinstance(item, dict)
                        else cls.anonymize(item) if isinstance(item, str) else item
                    )
                    for item in value
                ]
            else:
...
class AuditLogger:
    ...
    def log_access(
        self, user_id: str, action: str, phi_field: str, success: bool = True
    ) -> None:
        """Log PHI access event"""
        self.logger.info(
            f"PHI Access - User: {user_id}, Action: {action}, "
            f"Field: {phi_field}, Success: {success}"
        )
