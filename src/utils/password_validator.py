...
class PasswordValidator:
    ...
    COMMON_PASSWORDS = {
        ...
        'info', 'administrator', 'oracle', 'postgres', 'mysql'
    }
    ...
    def validate_password_strength(cls, password: str) -> ValidationResult:
        ...
        if password_bytes > cls.MAX_LENGTH:
            errors.append(f"Password exceeds {cls.MAX_LENGTH} bytes (bcrypt limitation). Current: {password_bytes} bytes")
        ...
        if cls._has_sequential_chars(password):
            errors.append("Password contains too many sequential or repeated characters")
    ...
    def _contains_dictionary_word(password: str, min_word_length: int = 4) -> bool:
        ...
        common_words = {
            'love', 'hate', 'good', 'bad', 'user', 'admin', 'test',
            'pass', 'word', 'secret', 'private', 'public', 'secure',
            'login', 'account', 'system', 'computer', 'internet',
            'email', 'phone', 'mobile', 'home', 'work', 'office'
        }
