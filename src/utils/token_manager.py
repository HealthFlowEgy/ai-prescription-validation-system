...
class TokenManager:
    ...
    def generate_access_token(
        self, user_id: int, role: str, expires_in: int = None
    ) -> Tuple[str, datetime]:
        """
    ...
    def decode_token(self, token: str, verify_blacklist: bool = True) -> Optional[Dict]:
        ...
        try:
            ...
            if verify_blacklist and self.is_token_blacklisted(payload.get("jti")):
                logger.warning(
                    f"Attempted use of blacklisted token: {payload.get('jti')}"
                )
                return None
            ...
            if payload.get("type") == "refresh":
                if not self._verify_token_binding(payload):
                    logger.warning(
                        f"Token binding verification failed for token: {payload.get('jti')}"
                    )
                    return None
    ...
    def revoke_token(self, jti: str, token_type: str = "access") -> bool:
        ...
        try:
            ...
            self.redis_client.setex(
                ...
                ttl,
                json.dumps(
                    {"revoked_at": datetime.utcnow().isoformat(), "type": token_type}
                ),
            )
    ...
    def _verify_token_binding(self, payload: Dict) -> bool:
        ...
        if token_device and request_device and token_device != request_device:
            logger.warning(
                f"Device ID mismatch: token={token_device}, request={request_device}"
            )
            return False
        ...
        if verify_ip:
            ...
            if token_ip and request_ip and token_ip != request_ip:
                logger.warning(
                    f"IP address mismatch: token={token_ip}, request={request_ip}"
                )
                return False
