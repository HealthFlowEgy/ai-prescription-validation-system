...
class ConflictError(APIError):
    ...
    def __init__(self, message="Resource conflict", details=None):
        super().__init__(
            message, status_code=409, error_code="CONFLICT_ERROR", details=details
        )
...
class DatabaseError(APIError):
    ...
    def __init__(self, message="Database operation failed", details=None):
        super().__init__(
            message, status_code=500, error_code="DATABASE_ERROR", details=details
        )
    ...
    def handle_operational_error(error):
        ...
        MonitoringService.capture_exception(
            error,
            {
                ...
                "request": {"endpoint": request.endpoint, "method": request.method},
            },
        )
...
def create_success_response(data=None, message=None, **kwargs):
    ...
    Returns:
        Tuple of (response, status_code)
    """
    response = {"status": "success", "timestamp": datetime.utcnow().isoformat()}
