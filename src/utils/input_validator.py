...
class FileValidator:
    ...
    def validate_upload(
        cls,
        file: FileStorage,
        file_category: str = 'prescription'
    ) -> ValidationResult:
        """
        ...
        if ext not in allowed_exts:
            errors.append(f"Invalid file type: {ext}. Allowed: {', '.join(allowed_exts)}")
        ...
        try:
            ...
            if mime not in allowed_mimes:
                errors.append(f"Invalid MIME type: {mime}. Allowed: {', '.join(allowed_mimes)}")
...
class InputValidator:
    ...
    def sanitize_input(cls, text: str, max_length: int = 1000) -> ValidationResult:
        ...
        if len(text) > max_length:
            errors.append(f"Input too long: {len(text)} characters. Maximum: {max_length}")
