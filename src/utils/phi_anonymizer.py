...
class PHIAnonymizer(logging.Filter):
    ...
    PHI_PATTERNS: List[Tuple[Pattern, str]] = [
        ...
        (re.compile(r'\bPatient:\s*[A-Z][a-z]+\s+[A-Z][a-z]+\b'), 'Patient: [NAME-REDACTED]'),
        (re.compile(r'\bDoctor:\s*[A-Z][a-z]+\s+[A-Z][a-z]+\b'), 'Doctor: [NAME-REDACTED]'),
        
        # Address patterns
        (re.compile(r'\b\d+\s+[A-Za-z\s]+(?:Street|St|Avenue|Ave|Road|Rd|Boulevard|Blvd)\b', re.IGNORECASE), '[ADDRESS-REDACTED]'),
