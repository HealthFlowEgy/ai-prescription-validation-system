...
class ConsentManagementService:
    ...
    def verify_consent(patient_id: str, consent_type: ConsentType) -> bool:
        ...
        consent = (
            Consent.query.filter_by(
                patient_id=patient_id, consent_type=consent_type.value
            )
    ...
    @staticmethod
    def revoke_consent(
        patient_id: str, consent_type: ConsentType, revoked_by: str
    ) -> bool:
        """
        ...
        if not consent:
            logger.warning(
                f"No active consent found to revoke for patient {patient_id}"
            )
            return False
    ...
    def get_consent_history(patient_id: str) -> List[Dict]:
        ...
        consents = (
            Consent.query.filter_by(patient_id=patient_id)
            .order_by(Consent.granted_at.desc())
            .all()
        )
        ...
        return [
            {
                ...
                "revoked_at": (
                    c.revoked_at.isoformat()
                    if hasattr(c, "revoked_at") and c.revoked_at
                    else None
                ),
    ...
    @staticmethod
    def check_data_access_authorization(
        patient_id: str, accessor_id: str, purpose: str
    ) -> bool:
        """
        ...
        if purpose == "treatment":
            return ConsentManagementService.verify_consent(
                patient_id, ConsentType.TREATMENT
            )
        elif purpose == "research":
            return ConsentManagementService.verify_consent(
                patient_id, ConsentType.RESEARCH
            )
        elif purpose == "ai_processing":
            return ConsentManagementService.verify_consent(
                patient_id, ConsentType.AI_PROCESSING
            )
        elif purpose == "data_sharing":
            return ConsentManagementService.verify_consent(
                patient_id, ConsentType.DATA_SHARING
            )
...
def revoke_consent():
    ...
    success = ConsentManagementService.revoke_consent(
        patient_id=patient_id, consent_type=consent_type, revoked_by=payload["user_id"]
    )
