            ...
            try:
                backup_data = {
                    'metadata': {
                        ...
                        'version': '1.0.0',
                        'database_uri': app.config.get('SQLALCHEMY_DATABASE_URI', 'sqlite:///app.db')
                    },
                    'users': [],
                    'prescriptions': [],
                    'medications': [],
                ...
                for prescription in prescriptions:
                    backup_data['prescriptions'].append({
                        ...
                        'input_format': prescription.input_format.value if prescription.input_format else None,
                        'file_path': prescription.file_path,
                        'original_filename': prescription.original_filename,
                        'file_size': prescription.file_size,
                        'processing_status': prescription.processing_status.value if prescription.processing_status else None,
                        'validation_status': prescription.validation_status.value if prescription.validation_status else None,
                ...
                print(f"  - Validation Results: {len(backup_data['validation_results'])}")
