...
def create_tables():
    ...
    with app.app_context():
        try:
            ...
            admin_user = User.query.filter_by(email='admin@prescriptionvalidator.com').first()
            if not admin_user:
                admin_user = User(
            ...
            test_user = User.query.filter_by(email='test@prescriptionvalidator.com').first()
            if not test_user:
                test_user = User(
                    name='Test User',
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Database management for Prescription Validation System')
    parser.add_argument('action', choices=['init', 'reset', 'drop', 'info'], 
