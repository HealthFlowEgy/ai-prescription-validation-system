    ...
    def check_cpu(self) -> HealthCheck:
        ...
        try:
            ...
            return HealthCheck(
                ...
                status=status,
                message=message,
                details={"percent_used": cpu_percent, "cpu_count": cpu_count},
            )
...
def create_health_check_routes(app):
    ...
    def readiness_check():
        ...
        if (
            db_check.status == HealthStatus.UNHEALTHY
            or redis_check.status == HealthStatus.UNHEALTHY
        ):
            return (
                jsonify(
                    {
...
def create_metrics_routes(app):
    ...
    def metrics():
        ...
        output.append(f"")
        output.append(
            f"# HELP http_request_duration_milliseconds HTTP request duration in milliseconds"
        )
        output.append(f"# TYPE http_request_duration_milliseconds summary")
        output.append(
            f'http_request_duration_milliseconds{{quantile="0.95"}} {metrics_data["p95_response_time_ms"]}'
        )
        output.append(
            f'http_request_duration_milliseconds{{quantile="0.99"}} {metrics_data["p99_response_time_ms"]}'
        )
        output.append(
            f'http_request_duration_milliseconds_sum {metrics_data["avg_response_time_ms"] * metrics_data["request_count"]}'
        )
        output.append(
            f'http_request_duration_milliseconds_count {metrics_data["request_count"]}'
        )
