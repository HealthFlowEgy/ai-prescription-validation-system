...
class Prescription(db.Model):
    ...
    processing_status = db.Column(
        db.Enum(ProcessingStatus), default=ProcessingStatus.UPLOADED
    )
    validation_status = db.Column(
        db.Enum(ValidationStatus), default=ValidationStatus.PENDING
    )
    ...
    medications = db.relationship(
        "Medication", backref="prescription", lazy=True, cascade="all, delete-orphan"
    )
    ...
    def to_dict(self):
        return {
            ...
            "validation_status": (
                self.validation_status.value if self.validation_status else None
            ),
            "patient_info": {
                ...
                "address": (
                    json.loads(self.patient_address) if self.patient_address else None
                ),
                "allergies": (
                    json.loads(self.patient_allergies) if self.patient_allergies else []
                ),
                "conditions": (
                    json.loads(self.patient_conditions)
                    if self.patient_conditions
                    else []
                ),
            },
            "prescriber_info": {
                ...
                "contact": (
                    json.loads(self.prescriber_contact)
                    if self.prescriber_contact
                    else None
                ),
            },
            "prescription_date": (
                self.prescription_date.isoformat() if self.prescription_date else None
            ),
...
class Medication(db.Model):
    ...
    prescription_id = db.Column(
        db.Integer, db.ForeignKey("prescriptions.id"), nullable=False
    )
...
class ValidationResult(db.Model):
    ...
    )  # drug_interaction, dosage, completeness, etc.
    ...
    def to_dict(self):
        return {
            ...
            "affected_fields": (
                json.loads(self.affected_fields) if self.affected_fields else []
            ),
            "recommendations": (
...
class AuditLog(db.Model):
    ...
    action = db.Column(
        db.String(100), nullable=False
    )  # upload, process, validate, approve, etc.
